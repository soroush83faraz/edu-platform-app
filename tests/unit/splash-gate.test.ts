...
import { SPLASH_BOOT_SCRIPT, SPLASH_FADE_MS, SPLASH_HOLD_MS, SPLASH_SEEN_KEY, SPLASH_TOTAL_MS, shouldShowSplash } from "@/lib/pwa/splash-gate";
...
describe("the timeline", () => {
  it("is hard-capped at 1.2 s — the CSS leave and the timer that clears up after it are the same number", () => {
    expect(SPLASH_TOTAL_MS).toBe(SPLASH_HOLD_MS + SPLASH_FADE_MS);
    expect(SPLASH_TOTAL_MS).toBeLessThanOrEqual(1200);
  });
});
